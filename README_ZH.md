...
指纹认证是OpenHarmony支持的一种生物认证执行器，按照协同认证定义的资源注册接口，将指纹认证相关资源信息注册到协同认证框架，并根据协同认证框架的调度，调用指纹认证HDI，完成指纹的录入，认证，删除等功能。
...
* 南向厂商适配指纹认证功能时，需实现IDL文件`//drivers/interface/fingerprint_auth` 中定义的接口，`//drivers/peripheral/fingerprint_auth`提供了接口的桩实现，供开发过程参考。指纹数据的提取、比对、存储和录入等过程应在尽可能安全的环境中实现，保证用户指纹数据的的安全保护达到系统内可以提供的最高安全级别（可信执行环境/安全芯片）。
